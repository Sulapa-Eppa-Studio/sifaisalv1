...
class ListSPMRequestAprovals extends ListRecords
{
    ...
    public function getTabs(): array
    {
        return [
            ...
            'Diproses' => Tab::make()
                ->modifyQueryUsing(fn(Builder $query) => $query->where('kpa_verification_status', 'in_progress')),
            'Disetujui' => Tab::make()
                ->modifyQueryUsing(fn(Builder $query) => $query->where('kpa_verification_status', 'approved')),
            'Ditolak' => Tab::make()
                ->modifyQueryUsing(fn(Builder $query) => $query->where('kpa_verification_status', 'rejected')),
        ];
    }
}
